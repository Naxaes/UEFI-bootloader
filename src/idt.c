...
#define GDT_ENTRY(access_, flags) (GlobalDescriptorEntry) {                   \
    ...
    .base_1=0,                                                                \
    .access=(access_),                                                        \
    .type_flags_and_limit_16_19=(u8) (((flags) << 4) & 0b11110000),           \
    .base_2=0                                                                 \
}
...
//    gdt[1] = GDT_ENTRY(0b10011010, 0b101);  // The kernel code segment descriptor.
//    gdt[2] = GDT_ENTRY(0b10010010, 0b101);  // The kernel data segment descriptor.
//    gdt[3] = GDT_ENTRY(0b10011010, 0b101);  // The user code segment descriptor.
//    gdt[4] = GDT_ENTRY(0b10010010, 0b101);  // The user data segment descriptor.
//    gdt[5] = GDT_ENTRY(0b10011010, 0b100);  // The TSS low section segment descriptor.
//    gdt[6] = GDT_ENTRY(0, 0);               // The TSS high section segment descriptor.

//    void* descriptor = get_descriptor();

//    struct __attribute__((packed)) { u16 size; u64 gdt; } gdt_descriptor = { sizeof(gdt)-1, (u64) gdt };
//    load_gdt();
...
//    tss.interrupt_stack_table[0] = (u64) (temp_stack + sizeof(temp_stack));
    ...
    struct __attribute__((packed)) { uint16_t size; uint32_t idt; } description  = { limit, base };
