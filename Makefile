...
EFI_CFLAGS :=-m64 -ffreestanding -O2 -Wall -Werror -Wextra -Wno-error=unused-parameter -Wno-error=unused-variable
EFI_LFLAGS :=-nostdlib -lgcc -shared -Wl,-dll -Wl,--subsystem,10 -e EfiMain -o
...
KERNEL_CFLAGS :=-fno-asynchronous-unwind-tables -mgeneral-regs-only -mno-red-zone -march=x86-64 --freestanding --std=c99 -Wall -Wextra -m64 -Og -ggdb -Wno-unused-parameter
KERNEL_LFLAGS :=-nostdlib -e _start
...
$(EFI_IMAGE): src/bootloader/efi_main.c $(BUILD_DIR)
	$(EFI_CC) $(EFI_CFLAGS) -c $< -o $(BUILD_DIR)/$@
	$(EFI_CC) $< $(EFI_LFLAGS) $(BUILD_DIR)/$@
...
kernel: src/kernel.c $(BUILD_DIR)
	$(NASM) src/setup.asm  $(KERNEL_NASM_FLAGS) -o $(BUILD_DIR)/setup.o
	$(NASM) src/x86_64.asm $(KERNEL_NASM_FLAGS) -o $(BUILD_DIR)/x86_64.o
	$(KERNEL_CC) $(KERNEL_CFLAGS) -c $< -o $(BUILD_DIR)/$@.o
	$(KERNEL_LD) $(KERNEL_LFLAGS) $(BUILD_DIR)/setup.o $(BUILD_DIR)/x86_64.o $(BUILD_DIR)/$@.o -o  $(BUILD_DIR)/$@
